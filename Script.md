NOTES FORM PINK for you to delete after reading: Assuming this is a script from which someone will eventually read aloud, you'll want to spell out large numbers (i.e., 'fifty thousand' instead of writing '50,000'), currencies ('thirty dollars' instead of '$30') as people are more easily able to read/speak simultaneously in this format. Same with abbreviations ('megabyte' vs 'MB'). Also, certain punctuation choices can influence how something is read aloud - a hyphen, for instance, can create a more natural pause when reading (as opposed to parentheses). <- see what I did there? Kappa. James, pay the man.

I'm not sure what 'net net' is on lines 19/49, so I left it in. :)
...
Most blockchains, Bitcoin included, have a target cadence for producing blocks.  This means that if more miners start creating blocks, the work gets harder so that the same number of blocks are actually produced.
...
Bitcoin Cash increased the blocksize from 1 megabyte to 8 megabytes.  This means Bitcoin Cash can handle 8 times as many transactions as Bitcoin.  
...
Huge blocks could be added to increase the operating a node or miner.  As a larger and larger hardware investment is required, less hobbyists participate and the network starts to be controlled by a limited number of large companies.  
